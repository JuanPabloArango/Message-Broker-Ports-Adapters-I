...
class TestPackageCreationRequestSchema:
    """Clase que encapsula las pruebas unitarias para la
    validación de DTOs externos."""

    def test_semi_empty_data(self) -> None:
        """Método que contiene la prueba unitaria encargada
        de validar que si un request llega vacío, la app lo entiende."""

        data = {"sender_id": "420"}
        result = PackageCreationRequestSchema().load(data)

        assert result == {"sender_id": "420",
                          "package_id": None}, "Valide que si el request body está vacío, no falle."
    ...
    def test_not_empty_data(self) -> None:
        ...
        assert result == {"sender_id": "321",
                          "package_id": None}, "Valide que se pasen las validaciones."
    ...
    def test_not_empty_data_fail(self) -> None:
        ...
        with pytest.raises(ValidationError):
            PackageCreationRequestSchema().load(data)
...
class TestPackageResponseSchema:
    ...
    def test_succesful_single_response(self) -> None:
        ...
        result = PackageResponseSchema(many = False).load(data)
    ...
    def test_succesful_many_responses(self) -> None:
        ...
        result = PackageResponseSchema(many = True).load(data)
    ...
    def test_failed_validation(self) -> None:
        ...
        with pytest.raises(ValidationError):
            PackageResponseSchema(many = False).load(data)
